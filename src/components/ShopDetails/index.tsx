...
import { ProductBySlug } from "@/types/product";
import Link from "next/link";

const ShopDetails = ({ product }: { product: ProductBySlug }) => {
  const { openPreviewModal } = usePreviewSlider();
  const [previewImg, setPreviewImg] = useState(0);
  const [seletedVariables, setSeletedVariables] = useState<string[]>(
    product.variables && product.variables.length > 0 ? [product.variables[0].optionId] : []
  );
  const [quantity, setQuantity] = useState(1);
  const [activeTab, setActiveTab] = useState("tabOne");
  const [currentPrice, setCurrentPrice] = useState(product.price);
  const [currentSalePrice, setCurrentSalePrice] = useState(product.salePrice);


  useEffect(() => {
    if (product.variables && product.variables.length > 0) {
      if (seletedVariables.length > 0) {
        const selectedVariable = product.variables.find(v => v.optionId === seletedVariables[0]);
        if (selectedVariable) {
          setCurrentPrice(selectedVariable.price);
          setCurrentSalePrice(selectedVariable.salePrice);
        }
      } else {
        setCurrentPrice(product.variables[0].price);
        setCurrentSalePrice(product.variables[0].salePrice);
      }
    }else{
      setCurrentPrice(product.price);
      setCurrentSalePrice(product.salePrice);
    }
  }, [seletedVariables, product.variables,product.price,product.salePrice]);
  ...
  return (
    <>
      <Breadcrumb title={"جزئیات محصول"} desc={product.name} pages={["جزئیات محصول"]} />
      {!product ? (
        "محصولی یافت نشد"
      ) : (
        <>
                          ...
                          product.images.length > 0 && (
                            <div className="lg:min-h-[512px] rounded-lg shadow-1 bg-gray-2 p-4 sm:p-7.5 relative flex items-center justify-center">
                              <div>
                                ...
                                <Image
                                  src={`${process.env.NEXT_PUBLIC_CATEGORY_IMAGE_BASE_URL}/${product.images[previewImg].formats.find(f => f.format == 'medium').filePath}`}
                                  alt="تصویر محصول"
                                  width={product.images[previewImg].formats.find(f => f.format == 'medium').width}
                                  height={product.images[previewImg].formats.find(f => f.format == 'medium').height}
                                />
                              </div>
                            </div>
                          ...
                          {product.images.map((item, key) => (
                            ...
                            >
                              <Image
                                width={50}
                                height={50}
                                src={`${process.env.NEXT_PUBLIC_CATEGORY_IMAGE_BASE_URL}/${item.formats.find(f => f.format == 'thumbnail').filePath}`}
                                alt="تصویر کوچک"
                              />
                            </button>
                          ))}
                ...
                <div className="max-w-[539px] w-full">
                  ...
                  <div className="flex flex-wrap items-center gap-5.5 mb-4.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm"> ({product.reviewsCount} مشتری دیدگاه خود را بیان کردند) </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {product?.stockStatus ===1 ? <span className="text-green"> موجود </span> : <span className="text-red"> ناموجود </span>}
                    </div>
                  </div>

                  <h3 className="font-medium text-custom-1 mb-4.5">
                    <span className="text-sm sm:text-base text-dark">
                      قیمت: <span className="text-2xl">{formatPrice(currentSalePrice)}</span> <span className="text-sm">تومان</span>
                    </span>
                    {
                      currentSalePrice !== currentPrice && (
                        <span className="line-through">
                          {formatPrice(currentPrice)} <span className="text-sm">تومان</span>
                        </span>
                      )
                    }
                  </h3>
                  ...
                  <ul className="flex flex-col gap-2">
                    <li className="flex items-center gap-2.5">
                      ...
                      برند : <Link href={`/brand/${product.brand}`}>{product.brand}</Link>
                    </li>
                    ...
                    {product.categories.length > 0 && (  
                        ...
                        دسته بندی : {product.categories.map((item, key) => (
                          <Link href={`/categories/${item.slug}`} key={key}>{item.name}</Link>
                        ))}
                      </li>
                      )
                    }  
                    ...
                    <div className="flex flex-col gap-4.5 border-y border-gray-3 mt-7.5 mb-9 py-9">
                      {
                        product.variables.length > 0 && <>
                          <div className="flex items-center gap-4">
                            <div className="flex items-center gap-4">
                              {product.variables.map((item, key) => (
                                <label
                                  key={key}
                                ...
                                >
                                  <div className="relative">
                                    <input
                                      ...
                                      id={item.optionName}
                                      className="sr-only"
                                      checked={seletedVariables.includes(item.optionId)}
                                      onChange={() => setSeletedVariables([item.optionId])}
                                    />

                                    <div
                                      className={`ml-1 flex h-4 w-4 items-center justify-center rounded border ${seletedVariables.includes(item.optionId)
                                        ? "border-blue bg-blue"
                                        : "border-gray-4"
                                        } `}
                                    >
                                      <span
                                        className={
                                          seletedVariables.includes(item.optionId)
                                            ? "opacity-100"
                                            : "opacity-0"
                                        }
                                      >
                ...
                >
                  <div className="max-w-[670px] w-full">
                    ...
                    product.tags.length > 0 && (
                      <div className="max-w-[670px] w-full">
                        <h6 className="font-medium text-sm text-dark mb-2">
                          برچسب ها:
                        </h6>
                        <div className="flex flex-wrap gap-2">
                          {product.tags.map((item, key) => (
                            <Link href={`/tags/${item.slug}`} key={key}>
                              <span className="text-xs text-dark bg-gray-1 px-2 py-1 rounded-md">
                                {item.name}
                              </span>
                            </Link>
                          ))}
                ...
                >
                  <div className="rounded-md even:bg-gray-1 flex py-4 px-4 sm:px-5">
                    <div className="max-w-[450px] min-w-[140px] w-full">
                      <p className="text-sm sm:text-base text-dark">برند</p>
                    </div>
                  ...
                  </div>
                  {
                    product.features.map((item, key) => (
                      <div key={key}>
                        <div className="rounded-md even:bg-gray-1 flex py-4 px-4 sm:px-5">
                          <div className="max-w-[450px] min-w-[140px] w-full">
                            <p className="text-sm sm:text-base text-dark">{item.featureName}</p>
                          </div>
                          <div className="w-full">
                            <p className="text-sm sm:text-base text-dark">{item.optionName}</p>
                          </div>
                        </div>
                ...
                >
                  <div className="max-w-[570px] w-full">
                    <h2 className="font-medium text-2xl text-dark mb-9">
                      {product.reviewsCount} دیدگاه
                    </h2>

                    <div className="flex flex-col gap-6">
                      {product.latestReviews &&
                        product.latestReviews.map((item, key) => (
                          <div key={key}>
                            <div className="rounded-xl bg-white shadow-1 p-4 sm:p-6">
                              ...
                              <div className="mt-6">
                                <span className="text-custom-sm">{item.title}</span>
                  ...
                  <div className="max-w-[550px] w-full">
                    <form>
                      ...
                      <p className="mb-6">
                        ایمیل شما نمایش داده نمی شود. فیلد های ضروری با * مشخص شده اند.
                      </p>
          ...
          </section>
          {
            product.relatedProducts && product.relatedProducts.length > 0 && (
              <RecentlyViewdItems products={product.relatedProducts} />
            )
          }
          <Newsletter />
        </>
